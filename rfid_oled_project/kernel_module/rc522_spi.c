...
static ssize_t rc522_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    ...
    // In a full driver, we would send Request & Anticoll commands here to read the UID.
    // For this demonstration, we return the hardware version to prove SPI works perfectly.
    ...
    // Only allow reading once per cat/open
    if (*ppos > 0)
        return 0;
...
static int rc522_probe(struct spi_device *spi)
{
    ...
    // Setup SPI mode (RC522 uses SPI Mode 0, MSB first)
    spi->mode = SPI_MODE_0;
    spi->bits_per_word = 8;
    spi->max_speed_hz = 1000000;
    ret = spi_setup(spi);
    if (ret < 0) {
        pr_err("SPI setup failed: %d\n", ret);
        return ret;
    }
    ...
    // Register character device /dev/rc522
    ...
    pr_info("RC522 SPI Driver successfully probed and device created.\n");
    return 0;
}
...
MODULE_DESCRIPTION("Custom SPI Character Driver for MFRC522 RFID");
